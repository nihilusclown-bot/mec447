...
if "qr_code" in query_params:
    ...
    if not df.empty:
        ...
        st.write(f"**Etapa atual:** {peca['etapa']}")
        st.write(f"**Responsável:** {peca['responsavel']}")
        st.write(f"**Data de cadastro:** {peca['data_cadastro']}")
        
        if peca["desenho_tecnico"]:
            st.image(peca["desenho_tecnico"], caption="Desenho Técnico", use_container_width=True)
        else:
            st.info("Nenhum desenho técnico cadastrado.")
        
        # Botão que força login e redireciona para Atualizar Status
        if st.button("🔄 Atualizar Status desta peça", type="primary"):
            st.session_state.scanned_qr = qr
            st.session_state.user = None  # força logout para login
            st.query_params.clear()
            st.rerun()
        
        st.stop()  # para a execução normal do app
    else:
